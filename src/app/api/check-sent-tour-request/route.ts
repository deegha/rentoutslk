...
export async function GET(req: Request) {
  try {
    ...
    const userTourRequestsArray = userData?.sentTourRequests || [];
    const sentTourRequests = [];
    ...
    if (
      ...
      for (const tourRequestId of userTourRequestsArray) {
        ...
        if (tourRequestDoc.exists()) {
          ...
          sentTourRequests.push({ id: tourRequestDoc.id, ...listingData });
        }
      }
    }

    return NextResponse.json({ sentTourRequests }, { status: 200 });
  } catch (error) {
